...
if [[ $size -ge 500 ]]; then
  echo "the total size( $size M) of the works is beyond the limit( 500 M) "
  echo "you should "
...
if [[ $zipsize -ge 100 ]]; then
  echo "the archive ($zipsize M) of the works is beyond the limit( 100 M) "
  echo "you should "
  echo " 1. clean the tmp file during the build process or "
  echo " 2. remove libraries and files those are not needed any more or"
  echo " 3. rework you code"
...
do
	...
	./random      	127.0.0.1 6000 127.0.0.2 6002 2222 0</dev/null 1>/dev/null 2>/dev/null &
	./random     	127.0.0.1 6000 127.0.0.3 6003 3333 0</dev/null 1>/dev/null 2>/dev/null &
	./fold     		127.0.0.1 6000 127.0.0.4 6004 4444 0</dev/null 1>/dev/null 2>/dev/null &
	./fold   		127.0.0.1 6000 127.0.0.5 6005 5555 0</dev/null 1>/dev/null 2>/dev/null &
	./fold		   	127.0.0.1 6000 127.0.0.6 6006 6666 0</dev/null 1>/dev/null 2>/dev/null &
	./fold			127.0.0.1 6000 127.0.0.7 6007 7777 0</dev/null 1>/dev/null 2>/dev/null &
